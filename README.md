...
| <a name="input_app"></a> [app](#input\_app) | An application to deploy with specific values Here you can specify: The name of the application to deploy,Chart name, Repository address, Chart version and path to ServiceAccount value in values.yaml | `map(any)` | <pre>{<br>  "chart": null,<br>  "deploy": true,<br>  "name": null,<br>  "path": null,<br>  "repository": null,<br>  "version": null<br>}</pre> | no |
...
| <a name="input_name"></a> [name](#input\_name) | Name for both service accounts. The GCP SA will be truncated to the first 30 chars if necessary. | `string` | `"example-name"` | no |
...
| <a name="input_values"></a> [values](#input\_values) | Chart extra values | `string` | n/a | yes |
